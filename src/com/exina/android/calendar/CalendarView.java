...
public class CalendarView extends ImageView {
    ...
    private static List<Calendar> appointments = new ArrayList<Calendar>();
    //    Drawable[][] mDecorations = new Drawable[6][7];
    Drawable mDecoration;

    static {
        appointments.add(new GregorianCalendar(2013, 0, 5));
        appointments.add(new GregorianCalendar(2013, 0, 10));
        appointments.add(new GregorianCalendar(2013, 0, 15));
    }
    ...
    private void initCells() {
        class _calendar {
            ...
            public _calendar(int d, boolean b, Calendar calendar) {
                day = d;
                thisMonth = b;
                if (calendar != null && appointments.contains(new GregorianCalendar(calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH), calendar.get(Calendar.DAY_OF_MONTH)))) {
                    appointment = true;
                }
            }
        ...
        for (int i = 0; i < tmp.length; i++) {
            ...
            for (int d = 0; d < n.length; d++) {
                if (mHelper.isWithinCurrentMonth(i, d)) {
                    ...
                    cellDate.set(mHelper.getYear(), mHelper.getMonth(), mHelper.getDayAt(i, d));
                    tmp[i][d] = new _calendar(n[d], true, cellDate);
                } else {
        ...
        for (int week = 0; week < mCells.length; week++) {
            for (int day = 0; day < mCells[week].length; day++) {
                if (tmp[week][day].thisMonth) {
                    ...
                    /*sprawdzenie czy w ten dzieñ jest umowione spotkanie*/
                    if(tmp[week][day].appointment) {
                        mCells[week][day] = new GreenCell(tmp[week][day].day, new Rect(Bound), CELL_TEXT_SIZE);
                    }
                } else {
    ...
    public class GreenCell extends Cell {
        public GreenCell(int dayOfMon, Rect rect, float s) {
            super(dayOfMon, rect, s);
            mPaint.setColor(0xdd115522);
            mPaint.setUnderlineText(true);
            mPaint.setShadowLayer(2f,0.3f, 0.3f, 0xdd333333);
        }
    }
    ...
    public static List<Calendar> getAppointments() {
        return appointments;
    }

    public static void setAppointments(List<Calendar> appointments) {
